...
async function createMessage(req, res) {
    ...
    try {
        ...
        const client_id = typeof req.body?.client_id === "string" ? req.body.client_id.trim() : "";
        ...
        if (client_id.length < 16 || client_id.length > 100) return res.status(400).json({ message: "Community identity is missing. Please refresh the page and try again." });
        ...
        const row = { id: await nextId("community_messages"), channel, display_name, message, reply_to_id, client_id, file_name, file_path, file_mime, file_size, created_at: new Date() };
...
async function editMessage(req, res) {
    try {
        ...
        const db = await getDB();
        const existing = await db.collection("community_messages").findOne({ id }, { projection: { _id: 0 } });

        if (!existing) {
            return res.status(404).json({ message: "Community message not found." });
        }

        const isAdmin = req.user?.role === "admin";
        const isOwner = client_id && existing.client_id === client_id;

        if (!isAdmin && !isOwner) {
            return res.status(403).json({ message: "You can edit only your own community messages." });
        }
        ...
        const updated = await db.collection("community_messages").findOne(
            { id },
            { projection: { _id: 0 } }
        );

        const reply = updated.reply_to_id
            ? await db.collection("community_messages").findOne(
                { id: updated.reply_to_id },
                { projection: { _id: 0 } }
            )
            : null;

        res.json({ message: formatMessage(updated, reply) });
    } catch (error) {
        console.error("EDIT COMMUNITY MESSAGE ERROR:", error);
        res.status(500).json({ message: "Unable to edit community message." });
    }
}
...
async function deleteMessage(req, res) {
    try {
        const id = Number(req.params.id);
        const client_id = typeof req.body?.client_id === "string"
        ...
        const isAdmin = req.user?.role === "admin";
        const isOwner = client_id && message.client_id === client_id;

        if (!isAdmin && !isOwner) {
            return res.status(403).json({ message: "You can delete only your own community messages." });
        }

        await db.collection("community_messages").deleteOne({ id });
        await db.collection("community_messages").updateMany(
            { reply_to_id: id },
        ...
        if (message.file_path) {
            try {
                ...
                console.error("COMMUNITY FILE DELETE ERROR:", error);
            }
        }
        ...
        res.status(500).json({ message: "Unable to delete community message." });
    }
}
